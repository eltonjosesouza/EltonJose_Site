...
def main():
    ...
    # Summary
    # If no locale files found, treat hardcoded strings as warnings [!] instead of errors [X]
    no_locales = any("No locale files found" in i for i in locale_result['issues'])

    critical_issues = 0
    for i in locale_result['issues']:
        if i.startswith("[X]"):
            critical_issues += 1
    ...
    for i in code_result['issues']:
        if i.startswith("[X]"):
            if no_locales:
                print(f"[!] (Warning Only - Single Language) {i[4:]}")
            else:
                critical_issues += 1
